...
parser.add_argument("-ed", "--eeg-dataset",
                    default=r"data\block\eeg_5_95_std.pth", help="EEG dataset path")  # 5-95Hz
...
parser.add_argument("-sp", "--splits-path",
                    default=r"data\block\block_splits_by_image_all.pth", help="splits path")  # All subjects
...
parser.add_argument("-sn", "--split-num", default=0,
                    type=int, help="split number")
...
parser.add_argument("-tl", "--time_low", default=20,
                    type=float, help="lowest time value")
parser.add_argument("-th", "--time_high", default=460,
                    type=float, help="highest time value")
...
parser.add_argument("-mp", "--model_params", default="",
                    nargs="*", help="list of key=value pairs of model options")
parser.add_argument("--pretrained_net", default="",
...
parser.add_argument("-lrdb", "--learning-rate-decay-by",
                    default=0.5, type=float, help="learning rate decay factor")
parser.add_argument("-lrde", "--learning-rate-decay-every",
                    default=10, type=int, help="learning rate decay period")
parser.add_argument("-dw", "--data-workers", default=4,
                    type=int, help="data loading workers")
parser.add_argument("-e", "--epochs", default=200,
                    type=int, help="training epochs")

# Save options
parser.add_argument("-sc", "--saveCheck", default=100,
                    type=int, help="learning rate")

# Backend options
parser.add_argument("--no-cuda", default=False,
                    help="disable CUDA", action="store_true")
    ...
    def __init__(self, eeg_signals_path):
        ...
        if opt.subject != 0:
            self.data = [loaded["dataset"][i] for i in range(
                len(loaded["dataset"])) if loaded["dataset"][i]["subject"] == opt.subject]
        else:
...
model_options = {key: int(value) if value.isdigit() else (float(value) if value[0].isdigit(
) else value) for (key, value) in [x.split("=") for x in opt.model_params]}
...
for epoch in range(1, opt.epochs+1):
    ...
    TrL, TrA, VL, VA, TeL, TeA = losses["train"]/counts["train"], accuracies["train"]/counts["train"], losses["val"] / \
        counts["val"], accuracies["val"]/counts["val"], losses["test"] / \
        counts["test"], accuracies["test"]/counts["test"]
    ...
    if epoch % opt.saveCheck == 0:
        torch.save(model, "%s__subject%d_epoch_%d.pth" %
                   (opt.model_type, opt.subject, epoch))
